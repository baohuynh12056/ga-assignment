...
class OneMaxProblem:
    def __init__(self, length: int = 100):
        self.length = length # [cite: 42]

    def fitness_function(self, genes: List[int]) -> float:
        return float(sum(genes)) # Điểm = số lượng bit 1 [cite: 43]
...
class KnapsackProblem:
    def __init__(self, num_items: int = 100, seed: int = 42):
        # Khởi tạo ngẫu nhiên weights và values bằng chung 1 seed để dễ test [cite: 51]
        random.seed(seed)
        self.num_items = num_items # [cite: 45]
        self.weights = [random.randint(1, 20) for _ in range(num_items)]
        self.values = [random.randint(10, 100) for _ in range(num_items)]
        self.capacity = sum(self.weights) * 0.4 # Sức chứa 40% [cite: 46]
    ...
    def fitness_function(self, genes: List[int]) -> float:
        ...
        if total_weight > self.capacity:
            return 0.0 # Bị phạt về 0 nếu vượt quá sức chứa [cite: 47]
